...
**理由:**
- 試行錯誤のコミット履歴を1つにまとめて、mainブランチの履歴を綺麗に保つ
- 機能単位での変更履歴が追いやすい
- リリースノート作成時に見やすい
...
```bash
# プルリクエストを作成
gh pr create --title "feat: 新機能追加" --body "説明文"
...
**注意:**
- GitHubのデフォルトは `--merge` ですが、このプロジェクトでは `--squash` を使用します
- ローカルとリモートのブランチが自動的に削除されます
