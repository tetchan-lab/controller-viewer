...
GitHub Pages を使うと、このリポジトリを **無料で Web サイトとして公開** できます。  
ローカルサーバーを立ち上げなくても OBS のブラウザソース URL として直接利用できるようになります。

### 公開前提の要件

| 項目 | 要件 |
|---|---|
| リポジトリの公開設定 | **Public**（GitHub Pro / Team / Enterprise では Private も可） |
| 公開ブランチ | `main`（または任意のブランチ） |
| 公開対象フォルダ | **`/ (root)`**（`index.html` がルートに存在するため） |

> **Note:** `docs/` フォルダを公開対象にすることもできますが、このプロジェクトでは `index.html` がルートにあるため、`/ (root)` を選択してください。

### 有効化手順

1. GitHub リポジトリの **Settings** タブを開く
2. 左サイドメニューの **Pages** をクリック
3. **Source** で **Deploy from a branch** を選択
4. **Branch** に `main`、フォルダに **`/ (root)`** を選んで **Save** をクリック

数秒〜数分後、以下の URL でアクセスできるようになります：

```
https://tetchan-lab.github.io/controller-viewer/
```

### 公開 URL の確認と更新

Settings → Pages を開くと、公開済みの場合は以下のように URL が表示されます：

```
Your site is live at https://tetchan-lab.github.io/controller-viewer/
```

**ページ更新の反映手順：**

1. ローカルでファイルを編集
2. `git commit` → `git push` で `main` ブランチにプッシュ
3. GitHub が自動でページを再ビルド（通常 1〜2 分）
4. ブラウザのキャッシュをクリア（`Ctrl + Shift + R`）して確認

> **Note:** ビルドの進捗は **Actions** タブ → `pages-build-deployment` ワークフローで確認できます。

### カスタムドメインの設定（オプション）

独自ドメイン（例: `controller.example.com`）を割り当てたい場合：

1. DNS プロバイダーで CNAME レコードを追加：  
   `controller.example.com` → `tetchan-lab.github.io`
2. Settings → Pages の **Custom domain** に独自ドメインを入力して **Save**
3. リポジトリルートに `CNAME` ファイルを作成し、ドメイン名のみを記載：

   ```
   controller.example.com
   ```

> **Note:** HTTPS は GitHub が自動で発行する Let's Encrypt 証明書により有効化されます（設定後数分〜数時間）。

### 制限・仕様

| 項目 | 上限 / 仕様 |
|---|---|
| 利用料金 | 無料（Public リポジトリ）、Private は GitHub Pro / Team / Enterprise で利用可 |
| リポジトリサイズ | 1 GB 以下を推奨（超過するとプッシュが警告・制限される場合あり） |
| 公開サイトサイズ | 1 GB 以下 |
| 月間転送量（Bandwidth） | 100 GB（ソフトリミット、超過時は GitHub から連絡） |
| ビルド回数 | 1 時間あたり 10 回まで |
| 単一ファイルサイズ | 100 MB 以下（Git の制限に準拠） |
| カスタムドメイン HTTPS | 自動発行（Let's Encrypt） |

> **参考:** [GitHub Pages の制限（公式ドキュメント）](https://docs.github.com/ja/pages/getting-started-with-github-pages/about-github-pages#usage-limits)
...
2. URL に以下のいずれかを入力：
   ...
   - GitHub Pages: `https://tetchan-lab.github.io/controller-viewer/`
   - コントローラー指定: `https://tetchan-lab.github.io/controller-viewer/?controller=dualsense`
