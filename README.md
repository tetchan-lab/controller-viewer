...
- **すべてのSTANDARD GAMEPADデバイスに対応**
  - STANDARD GAMEPAD規格に準拠したコントローラーなら、どのメーカー・モデルでも動作
  - DualSense、Fighting Stick Miniの設定を自動適用、または手動選択可能
- **デバイス選択機能**
  - 複数のコントローラーを同時接続した際、特定のデバイスだけを表示可能
  - UIドロップダウンでの選択、またはURLクエリパラメーター（`?device=`）での指定に対応
...
- **キーボード/マウス入力対応**（ゲームパッド未接続でも動作）
  - W/A/S/D、アローキー、Space、マウスクリックなどでコントローラー入力をシミュレート
  - 実ゲームパッドと併用可能（OR マージ）
  - テキスト入力中は自動的に無効化
...
> **Note:** テンキー（Numpad8/2/4/6）も右スティックとして使用できます（Mac キーボード非搭載環境向けにアローキーを優先）。

### 注意事項

- テキスト入力欄（`<input>`, `<textarea>`, `<select>`）にフォーカスがある場合、キーボード入力は無効化されます
...
### ✅ すべてのSTANDARD GAMEPADに対応

**Gamepad API の `mapping: "standard"` を持つコントローラーなら、メーカー・モデルを問わず動作します。**  
現在の実装では、どのSTANDARD GAMEPADデバイスを接続してもDualSenseまたはFighting Stick Miniの設定が適用されます。
...
上記以外のSTANDARD GAMEPADデバイス（Xbox Elite Controller、Nintendo Switch Pro Controller、サードパーティ製コントローラーなど）も問題なく動作します。  
接続後、画面上部のボタンで手動でマッピングを選択してください。
...
> **Note:**  
 `file://` プロトコルでは Gamepad API が使えないブラウザがあり、Web Audio API も使えません。ローカルサーバーの使用を推奨します。    
...
#### 複数のコントローラーを同時使用する場合（OBS用）

**複数のコントローラーを同時接続している環境でOBSに個別表示したい場合**は、`?device=` パラメーターを使用します。

##### デバイス指定方法

`?device=` パラメーターには以下の3つの形式が使用できます：

1. **デバイスインデックス**（接続順の番号: 0, 1, 2...）
   ```
   ?controller=dualsense&device=0
   ```

2. **デバイスID部分一致**（デバイス名の一部）
   ```
   ?controller=dualsense&device=dualsense
   ?controller=fightingStickMini&device=xbox
   ```

3. **ベンダーID:プロダクトID**（USB識別子）
   ```
   ?controller=dualsense&device=054c:0ce6
   ```

##### 使用例：2つのブラウザソースで異なるコントローラーを表示

**ブラウザソース1（DualSense専用）：**
```
https://tetchan-lab.github.io/controller-viewer/?controller=dualsense&device=0
```
- 幅: 800、高さ: 565
- デバイスインデックス 0 番のコントローラーのみ反応

**ブラウザソース2（Fighting Stick Mini専用）：**
```
https://tetchan-lab.github.io/controller-viewer/?controller=fightingStickMini&device=1
```
- 幅: 800、高さ: 457
- デバイスインデックス 1 番のコントローラーのみ反応

> **Note:** デバイスインデックスはブラウザの接続順に割り当てられます。  
> OBSのブラウザソースと通常ブラウザでは接続順が異なる場合があるため、  
> OBS側で実際に接続して番号を確認してください。
>
> より確実にデバイスを特定したい場合は、ベンダーID:プロダクトID形式を推奨します。  
> DualSenseの場合: `?device=054c:0ce6`
...
- **[2] デバイス名**：デバイスインデックス 2 番のコントローラーのみ
...
詳細は [複数のコントローラーを同時使用する場合（OBS用）](#複数のコントローラーを同時使用する場合obs用) をご覧ください。
